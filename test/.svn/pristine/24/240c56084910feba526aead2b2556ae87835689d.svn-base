...
class Task extends BaseController {
    ...
    public function gold_rate(){
        ...
        $content = [
            ...
            'str1'      =>  '每自然周2天下单获赠' . $this->gold['one_gold'] . '金币;3天下单可再获赠' . $this->gold['two_gold'] . '金币！金币支持兑换礼品啦，赶快参与吧！'
        ];
        ...
        if($task_info['week_day_count'] == 2){
            ...
            if($is_week == 0){
                $content = [
                    'str1'      =>  '本周累计下单2天，无再次领取机会，下周将重新计算下单天数，请注意活动进度及时下单领取！'
                ];
            }else{//今天为非周天
                $content = [
                    'str1'      =>  '本周已累计下单2天获赠' . $this->gold['one_gold'] . '金币，再下单1天获赠' . $this->gold['two_gold'] . '金币，下周将重新计算下单天数，请注意活动进度及时下单领取！',
                ];
            }
            ...
            if(isset($param['orderid'])  && !isset($is_week_order)){
                $content = [
                    'str1'      =>  '上周累计下单2天，无再次领取机会，本周将重新计算下单天数，请注意活动进度及时下单领取！'
                ];
            }
        }
        ...
        if($task_info['week_day_count'] >= 3){
            ...
            $content = [
                'str1'      =>  '本周累计下单'.$task_info['week_day_count'].'天，已获取全部金币奖励,下周将重新计算下单天数，邀您继续参与活动！'
            ];

            //如果是订单只调用
            if(isset($param['orderid']) && !isset($is_week_order)){
                $content = [
                    'str1'      =>  '上周累计下单'.$task_info['week_day_count'].'天，已获取全部金币奖励,本周将重新计算下单天数，邀您继续参与活动！'
                ];
            }
        }
    ...
    public function gold_rate_lists(){
        ...
        foreach ($gold_lists as $k => $v){
            ...
            if($v['week_day_count'] == 2){
                ...
                if($v['gift2'] == 0){
                    //$content = '您有1份好礼待领取！';
                    $content = '您有' . $this->gold['one_gold'] . '金币待领取！';
                }
            }
            //下单三次及以上
            if($v['week_day_count'] >= 3){
                ...
                if($v['gift2'] == 0 && $v['gift3'] == 0){
                    ...
                    $content = '您有'.$this->gold['two_gold'].'金币待领取！';
                }
            }
