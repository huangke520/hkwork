...
//        potential_customer
    ...
    public function showPanel(){
        ...
        //查询店铺信息
        $sup_data = Db::connect('db_mini_mall')->table('ims_yd_supplier')->where('id = '.$sup_id)->find();
        $panel_data = array();
        if((!empty($sup_data)) && ($sup_data['cai_gou'] == 1)){
            //根据店铺ID查询点位ID
            $potential_customer = Db::connect('db_btj_new')->table('potential_customer')->where('supplier_id = '.$sup_id)->field('id')->find();
            $potential_customer_id = !empty($potential_customer['id']) ? $potential_customer['id'] : 0;
            $panel_data['panel_name'] = '商品标记';
            $panel_data['url'] = 'https://btj.yundian168.com/biz/bd1/index.html#/goodSign?customerId='.$potential_customer_id;//链接地址
//            $panel_data['url'] = 'https://btj.yundian168.com/biz/bd1/index.html#/goodSign?customerId=179116';//链接地址
            $panel_data['is_show'] = 1;//1：展示，0：不展示
        }
//        $return_data[] = $panel_data;
        sdk_return($panel_data,1,'返回成功');
    }
}
