...
//        $goods = $this->shop_goods_model->getSkuChannelGoods($where);
        $goods = Db::connect('db_mini_mall')
            ->field(['g.id','g.title','g.thumb','g.sale_pirce','g.marketprice','g.skuid','IFNULL(cs.channel_count, 0) as channel_count','g.first_order_price','g.level2_price','g.level3_price','g.level4_price','g.level5_price','g.is_level_goods'])
...
//                $cate_goods = $this->shop_goods_model->getCateGoods($cate_where, 5 - count($goods));
                $cate_goods = DB::connect('db_mini_mall')->field(['id','title','thumb','sale_pirce','marketprice','skuid','first_order_price','level2_price','level3_price','level4_price','level5_price','is_level_goods'])->table('ims_ewei_shop_goods')->where($cate_where)->limit((5 - count($goods)))->select();
        ...
        //查询用户在当前店铺是否有订单
        $is_have_order = Db::connect('db_mini_mall')->table('ims_ewei_shop_order')->where('supplier_id = '.$sup_id.' and (status = 0 or status = 1 or status = 2 or status = 3) and openid = "'.$user_openid.'"')->count();
        ...
        foreach ($goods as $k => $v){
            ...
            $goods[ $k ]['level_msg1'] = '';
            $goods[ $k ]['level_msg2'] = '';
            if(!empty($goods[ $k ]['is_level_goods'])){
                $goods[ $k ]['level_price'] = $level_price;
                if(!empty($is_have_order)){
                    if (!empty($level_price)){
                        $goods[ $k ]['sale_pirce'] = $goods[ $k ]['level'.$user_level.'_price'];
                        if(($user_level != 5) && (!empty($next_level_price)) && (!empty($next_level))){
                            $goods[ $k ]['level_msg1'] = '升级可享'.$next_level_price.':￥'.$goods[ $k ]['level'.$next_level.'_price'];
                            $goods[ $k ]['level_msg2'] = '最高可享钻卡价:￥'.$goods[ $k ]['level5_price'];
                        }
                    }else{
                        $goods[ $k ]['level_msg1'] = '升级可享普卡价:￥'.$goods[ $k ]['level2_price'];
                        $goods[ $k ]['level_msg2'] = '最高可享钻卡价:￥'.$goods[ $k ]['level5_price'];
                    }
                }else{
                    $goods[ $k ]['level_price'] = '首单价';
                    $goods[ $k ]['sale_pirce'] = $goods[ $k ]['first_order_price'];
                }
            }
...
//                var_dump($location.$result['geocodes'][0]['province'].",".$result['geocodes'][0]['city'].",".$result['geocodes'][0]['district'].":".$i++);
        ...
        $area_where = '';
        if(!empty($area_id)){
            $area_where = ' and ((a.goods_area = 0) or (a.goods_area <> 0 and a.goods_area like "%'.$area_id.'%"))';
        }else{
            $area_where = ' and (a.goods_area = 0) ';
        }
//        order by g.sales desc,g.createtime desc,g.id desc
        $goods_data = Db::connect('db_mini_mall')->table('ims_ewei_shop_goods')->alias('a')->leftJoin('ims_yd_supplier_goods b','a.id = b.goods_id')->field('a.id,a.title,a.thumb,a.sale_pirce,a.marketprice,a.first_order_price,a.level2_price,a.level3_price,a.level4_price,a.level5_price,a.is_level_goods')->where(' a.status = 1 and b.status = 1 and a.deleted = 0 and a.total > 0 and a.id <> 68293 and a.isrecommand = 1 '.$whereOr.$area_where)->order(['a.sales'=>'desc','a.createtime'=>'desc','a.id'=>'desc'])->limit(6)->select();
        ...
        if(!empty(count($goods_data))){
            foreach ($goods_data as $k => $one){
//判断会员价
                $goods_data[ $k ]['level_price'] = '';
                $goods_data[ $k ]['level_msg1'] = '';
                $goods_data[ $k ]['level_msg2'] = '';
                if(!empty($goods_data[ $k ]['is_level_goods'])){
                    $goods_data[ $k ]['level_price'] = $level_price;
                    if(!empty($is_have_order)){
                        if (!empty($level_price)){
                            $goods_data[ $k ]['sale_pirce'] = $goods_data[ $k ]['level'.$user_level.'_price'];
                            if(($user_level != 5) && (!empty($next_level_price)) && (!empty($next_level))){
                                $goods_data[ $k ]['level_msg1'] = '升级可享'.$next_level_price.':￥'.$goods_data[ $k ]['level'.$next_level.'_price'];
                                $goods_data[ $k ]['level_msg2'] = '最高可享钻卡价:￥'.$goods_data[ $k ]['level5_price'];
                            }
                        }else{
                            $goods_data[ $k ]['level_msg1'] = '升级可享普卡价:￥'.$goods_data[ $k ]['level2_price'];
                            $goods_data[ $k ]['level_msg2'] = '最高可享钻卡价:￥'.$goods_data[ $k ]['level5_price'];
                        }
                    }else{
                        $goods_data[ $k ]['level_price'] = '首单价';
                        $goods_data[ $k ]['sale_pirce'] = $goods_data[ $k ]['first_order_price'];
                    }
                }
            }
        }
    ...
    public function likeGoods(){
        ...
        $sort = !empty($param['sort']) ? $param['sort']: 1;//1:购买时间排序，2：购买次数排序
        $bb_cate2_where = '';
        if(!empty($bb_cate2)){
        ...
        $pageSize = 100000;
        if($sort == 1){
            $order_where = ' ORDER BY a.createtime desc';
        }else{
            $order_where = '';
        }
        $data = Db::connect('db_mini_mall')->table('ims_ewei_shop_order_goods')->alias('a')->leftJoin('ims_ewei_shop_order b','a.orderid = b.id')->leftJoin('ims_ewei_shop_goods c','a.goodsid = c.id')->where('b.openid = "'.$user_openid.'" and b.supplier_id = '.$sup_id.' and (b.status = 1 or b.status = 3)'.$bb_cate2_where.' and a.total <> 0 and a.is_activity = 0')->field('a.id,a.goodsid,b.openid,c.title,c.thumb,c.skuid,c.bb_start_count,c.bb_end_count,c.bb_step,c.salesreal,c.sale_pirce as marketprice,c.first_order_price,c.level2_price,c.level3_price,c.level4_price,c.level5_price,c.is_level_goods')->group('a.goodsid')->paginate($pageSize)->toArray();
//        $data = Db::connect('db_mini_mall')->query("SELECT a.id,a.goodsid,c.title,c.thumb,c.skuid,c.bb_start_count,c.bb_end_count,c.bb_step,c.salesreal,c.sale_pirce as marketprice,c.first_order_price,c.level2_price,c.level3_price,c.level4_price,c.level5_price,c.is_level_goods from (SELECT * from ims_ewei_shop_order_goods WHERE orderid in(SELECT id from ims_ewei_shop_order where openid = '{$user_openid}' and (`status` = 1 or `status` = 3) and supplier_id = {$sup_id}) and total > 0 and is_activity = 0 ORDER BY createtime) as a LEFT JOIN ims_ewei_shop_goods as c on a.goodsid = c.id GROUP BY a.goodsid {$order_where}");
//'first_order_price','level2_price','level3_price','level4_price','level5_price','is_level_goods'
        //获取会员等级
        $user_level = getMemberLevel($user_openid,$sup_id);
        if($user_level > 1){
            $level_arr = array(2=>'普卡价',3=>'银卡价',4=>'金卡价',5=>'钻卡价');
...
//            $goods_data = $data;
            foreach ($goods_data as $key => $value){
                ...
                $value['level_msg1'] = '';
                $value['level_msg2'] = '';
                if(!empty($value['is_level_goods'])){
                    $value['level_price'] = $level_price;
                    if(!empty($is_have_order)){
                        if (!empty($level_price)){
                            $value['sale_pirce'] = $value['level'.$user_level.'_price'];
                            if(($user_level != 5) && (!empty($next_level_price)) && (!empty($next_level))){
                                $value['level_msg1'] = '升级可享'.$next_level_price.':￥'.$value['level'.$next_level.'_price'];
                                $value['level_msg2'] = '最高可享钻卡价:￥'.$value['level5_price'];
                            }
                        }else{
                            $value['level_msg1'] = '升级可享普卡价:￥'.$value['level2_price'];
                            $value['level_msg2'] = '最高可享钻卡价:￥'.$value['level5_price'];
                        }
                    }else{
                        $value['level_price'] = '首单价';
                        $value['sale_pirce'] = $value['first_order_price'];
                    }
                }
...
//                $arr = Db::connect('db_mini_mall')->table('ims_ewei_shop_member_cart')->where('openid',$openids)->where('deleted',0)->where('goodsid',$v['id'])->find();
                ...
                //查询当前商品订购了多少次，多少个
                $goods_sum_arr = Db::connect('db_mini_mall')->table('ims_ewei_shop_order_goods')->alias('a')->leftJoin('ims_ewei_shop_order b','a.orderid = b.id')->where('b.openid = "'.$user_openid.'" and b.supplier_id = '.$sup_id.' and (b.status = 1 or b.status = 3) and a.goodsid = '.$value['goodsid'].' and a.total <> 0')->field('count(a.id) as goods_sum,sum(total) as allTotal,a.createtime')->order('a.createtime','desc')->find();
                ...
                //查询当前商品最新的订购时间
                $goods_time_arr = Db::connect('db_mini_mall')->table('ims_ewei_shop_order_goods')->alias('a')->leftJoin('ims_ewei_shop_order b','a.orderid = b.id')->where('b.openid = "'.$user_openid.'" and b.supplier_id = '.$sup_id.' and (b.status = 1 or b.status = 3) and a.goodsid = '.$value['goodsid'].' and a.total <> 0')->field('a.createtime')->order('a.createtime','desc')->find();
                $value['createtime'] = !empty($goods_time_arr['createtime']) ? $goods_time_arr['createtime'] : 0;//订购时间
        ...
        if($sort == 2 && !empty(count($return_data))){
            //按照购买次数排序
            $return_data = arraySequence($return_data,'goods_sum');
        }elseif($sort == 1 && !empty(count($return_data))){
            //按照订购时间
            $return_data = arraySequence($return_data,'createtime');
        }
        sdk_return($return_data,0,'success');
    }
...
//        $data = Db::connect('db_mini_mall')->table('ims_ewei_shop_order_goods')->alias('a')->leftJoin('ims_ewei_shop_order b','a.orderid = b.id')->leftJoin('ims_ewei_shop_goods c','a.goodsid = c.id')->where('b.openid = "'.$user_openid.'" and b.supplier_id = '.$sup_id.' and (b.status = 1 or b.status = 3) and c.bb_cate2 <> 0 and a.total <> 0')->field('a.id,a.goodsid,b.openid,c.bb_cate2,count(c.bb_cate2) as cate_num')->group('c.bb_cate2')->select();
        $goods_data = Db::connect('db_mini_mall')->table('ims_ewei_shop_order_goods')->alias('a')->leftJoin('ims_ewei_shop_order b','a.orderid = b.id')->field('a.goodsid')->where('b.supplier_id = '.$sup_id.' and (b.status = 1 or b.status = 3) and a.total <> 0 and b.openid = "'.$user_openid.'" and a.is_activity = 0')->group('a.goodsid')->select();
        $goods_id_arr = [];
        if(!empty(count($goods_data))){
            foreach ($goods_data as $one_g){
                $goods_id_arr[] = $one_g['goodsid'];
            }
        }
        $data = Db::connect('db_mini_mall')->table('ims_ewei_shop_goods')->where([['id','in',$goods_id_arr]])->where('bb_cate2 <> 0 ')->field('id,id as goodsid,bb_cate2,count(id) as cate_num')->group('bb_cate2')->select();
//        echo Db::connect('db_mini_mall')->getLastSql();exit;
        ...
        if($all_cate_num != 0){
            ...
            $return_data = arraySequence($cate_data,'cate_num');
        }
        sdk_return($return_data,0,'success');
    }
