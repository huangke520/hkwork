...
//        echo Db::connect('db_mini_mall')->table('ims_ewei_shop_goods')->getLastSql();exit;
        ...
        //查询用户在当前店铺是否有订单
        $is_have_order = Db::connect('db_mini_mall')->table('ims_ewei_shop_order')->where('supplier_id = '.$sup_id.' and (status = 0 or status = 1 or status = 2 or status = 3) and openid = "'.$openids.'"')->count();
        $data_arr = array();
...
//                $arr = Db::connect('db_mini_mall')->table('ims_ewei_shop_member_cart')->where('openid',$openids)->where('deleted',0)->where('goodsid',$v['id'])->find();
                ...
                $v['level_msg1'] = '';
                $v['level_msg2'] = '';
                if(!empty($v['is_level_goods'])){
                    $v['level_price'] = $level_price;
                    if(!empty($is_have_order)){
                        if (!empty($level_price)){
                            $v['supplier_price'] = $v['level'.$user_level.'_price'];
                            if(($user_level != 5) && (!empty($next_level_price)) && (!empty($next_level))){
                                $v['level_msg1'] = '升级可享'.$next_level_price.':￥'.$v['level'.$next_level.'_price'];
                                $v['level_msg2'] = '最高可享钻卡价:￥'.$v['level5_price'];
                            }
                        }else{
                            $v['level_msg1'] = '升级可享普卡价:￥'.$v['level2_price'];
                            $v['level_msg2'] = '最高可享钻卡价:￥'.$v['level5_price'];
                        }
                    }else{
                        $v['level_price'] = '首单价';
                        $v['supplier_price'] = $v['first_order_price'];
                    }
                }
...
//                $v['goods_channel_count'] = !empty($hbsj_data_2['channel_count']) ? $hbsj_data_2['channel_count'] : 0;
                ...
                //库存处理（是否启用erp库存）
                if($v['sale_type'] == 1){
                    $v['total'] = $v['erp_total'] < $v['total'] ? $v['erp_total'] : $v['total'];
                }
...
//        $rec_brand_arr = Db::connect('db_mini_mall')->query("SELECT c.*,sum(c.sku_count) as new_count from (SELECT a.*,sum(a.sku_count) as new_sku_count,b.b_name from ims_bb_supplier_cate_brand as a LEFT JOIN ims_bb_brand as b on a.brand_id = b.id WHERE a.sup_id = {$sup_id} GROUP BY a.brand_id,a.cate_id) as c GROUP BY c.brand_id order by new_count desc limit 6");
        $rec_brand_arr = Db::connect('db_mini_mall')->query("SELECT c.*, sum(c.sku_count) AS new_count FROM ( SELECT a.*, sum(a.sku_count) AS new_sku_count, b.b_name FROM ims_bb_supplier_cate_brand AS a LEFT JOIN ims_bb_brand AS b ON a.brand_id = b.id WHERE a.sup_id = {$sup_id} AND ( b.b_name = '康师傅' OR b.b_name = '统一' OR b.b_name = '可口可乐' OR b.b_name = '百事可乐' OR b.b_name = '农夫山泉' OR b.b_name = '香飘飘' ) GROUP BY a.brand_id, a.cate_id ) AS c GROUP BY c.brand_id LIMIT 6");
        $custom_arr = ['康师傅'=>0,'统一'=>1,'可口可乐'=>2,'百事可乐'=>3,'农夫山泉'=>4,'香飘飘'=>5];
...
//                2019年10月9日14:30:56
                $value['cate_bb1'] = 0;
                $value['cate_bb2'] = 0;
                $cus = $custom_arr[$value['b_name']];
                $rec_brand_data[$cus] = $value;
            }
//            $rec_brand_data = arraySequence($rec_brand_data,'sup_count');
        }

        $rec_brand_data = array_values($rec_brand_data);
