...
//                $status = '已确认，待支付';
            ...
            foreach ($order_goods_list as $key => $value) {
                $goodsids[] = $value['is_activity'] > 0 ? $value['is_activity'] : $value['goodsid'];
            }
            $goodsnum = Db::connect('db_mini_mall')
                ->table('ims_ewei_shop_order_goods')
                ->alias('a')
                ->leftJoin('ims_ewei_shop_order b', 'a.orderid = b.id')
                ->field('a.goodsid,sum(a.total) as totalsum')
                ->where('a.goodsid','IN',array_unique($goodsids))
                ->where('b.status',0)
                ->where('b.delivery_status','IN','1,2,4,5')
                ->group('a.goodsid')
                ->select();
            $goodsnums = [];
            foreach ($goodsnum as &$v){
                $goodsnums[$v['goodsid']] = $v['totalsum'];
            }
            foreach ($order_goods_list as $key => $value) {
                $goodsid = $value['is_activity'] > 0 ? $value['is_activity'] : $value['goodsid'];
                $erp_total = $this->erpStorage($goodsid);
                if(isset($erp_total[$goodsid])){
                    $value['kenum'] = isset($goodsnums[$goodsid]) ? $goodsnums[$goodsid] : 0;
                    $value['erp_total'] = $erp_total[$goodsid]['qty'] - $erp_total[$goodsid]['assign_qty'] - $erp_total[$goodsid]['unassign_qty'] - $value['kenum'];
                }else{
                    $value['erp_total'] = 0;
                }
...
//                $data['goods_price'] = $count_money;//总计价格
     ...
     * desc 动作说明
     */
    private function deliveryAction($uid,$action,$orderid = 0,$goodsid = 0,$type = 0,$desc = '')
    {
        $data = [];
        $data['uid'] = $uid;//用户id
        $data['action'] = $action;//动作id 1分拣2装车3划单4签到5送达6拒收7排线8发车9回库

        $lineid = Db::connect('db_mini_mall')->table('ims_ewei_drivers_orders')->where('order_id',$orderid)->value('line_id');
        ...
        //获取当天操作者最早工作时间
        $startime = Db::connect('db_mini_mall')->table('ims_delivery_action_log')->where('uid',$uid)->where('createtime','>',strtotime(date('Y-m-d')))->min('createtime');
