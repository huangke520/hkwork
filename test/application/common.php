 ...
 * check_pram
 */
if (!function_exists('check_pram')) {
    function check_pram($pram)
    {
        return $pram = str_replace('__', ' ', $pram);
    }
}
 ...
 * echo_html
 */
if (!function_exists('echo_html')) {
    function echo_html($res)
    {
    ...
    function get_task_gold($type = 1)
    {
        switch ($type) {
            case 2:
                ...
                break;
            case 3:
                $gold = 3000;//2019年11月12日10:08:07，吴文江说改成3000
                break;
            default:
                $gold = 0;
                break;
...
/**
 * 临时写日志，人工方便调试，不要上线后一直写入
 */
if (!function_exists('sys_log')) {
    function sys_log($info = '', $type = 0)
    {
        ...
        $arr_log['log_type'] = $type;
        $arr_log['log_info'] = $str_info;
        $arr_log['create_time'] = time();
        $m_SystemDebugLog->insertInfo($arr_log);
    }
}
 ...
 */
if(!function_exists('getMemberLevel')){
    function getMemberLevel($openid = '',$sup_id = ''){
        //查询当月会员的等级
//        $level_month_model = new MemberMonthLevel();
        $level_month_model = new \app\api\model\ydxq\MemberMonthLevel();
        $level_where = [
            ['openid','=',$openid],
            ['sup_id','=',$sup_id],
        ];
