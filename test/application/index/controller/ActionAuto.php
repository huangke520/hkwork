...
//关闭URL请求
    ...
    /**
     * 清洗10月份ims_ewei_shop_order表中bd_money的值（第一版）
     * 统计2小时内订单业务员业提成金额
     */
...
//    }
    ...
    /**
     * 清洗ims_ewei_shop_order表中bd_money的值（第二版）
     * 统计2小时内订单业务员业提成金额
     */
    public function index_zt46() {
        //set_time_limit(0);
        ...
        //$day_start = strtotime('2019-10-01');
        //确认时间
        $day_start = time() - 60 * 60 * 2;//两小时之前
//        $start_time = strtotime('2019-11');
//        $end_time = strtotime(date('Y-m-01 00:00:00',strtotime('+1 month',$start_time)));
        //查询订单商品
        $order_goods_list = Db::connect('db_mini_mall')->query("SELECT id,price,goodsid,orderid from ims_ewei_shop_order_goods WHERE orderid in(SELECT `id` FROM `ims_ewei_shop_order` WHERE ( supplier_id = 461 and (status = 1 or status = 3) and paytime >= {$day_start})) and total > 0");
        //查询商品分类
        $goods_list = Db::connect('db_mini_mall')->query("SELECT id,title,bb_cate1,bb_cate2 from ims_ewei_shop_goods where id in(SELECT goodsid from ims_ewei_shop_order_goods WHERE orderid in(SELECT `id` FROM `ims_ewei_shop_order` WHERE ( supplier_id = 461 and (status = 1 or status = 3) and paytime >= {$day_start} )) and total > 0)");
//        echo Db::connect('db_mini_mall')->getLastSql();exit;
        $goods_cate_arr = [];
        if(!empty(count($goods_list))){
...
//            foreach ($order_goods_list as $key => $value) {
        ...
        //更新订单提成
        if(!empty(count($order_bd_money))){
            foreach ($order_bd_money as $key_order_id => $value_bd_money) {
                $value_bd_money = round($value_bd_money, 2);
                Db::connect('db_mini_mall')->table('ims_ewei_shop_order')->where('id = ' . $key_order_id)->update(['bd_money' => $value_bd_money]);
            }
        }
        echo time() . "<br>";
    }
